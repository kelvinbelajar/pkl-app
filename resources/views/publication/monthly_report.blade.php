...
<body>
  <section class="sheet">
    ...
    <h1 style="text-align: center;">LAPORAN DATA PUBLICATION BULANAN</h1>
      ...
      @if ($publications->count() > 0)
        @foreach ($publications as $data)
          <tr align="center">
                ...
                <td>
                  <img src="{{ $data->project->picture }}" alt="Picture" width="100" height="100">
                </td>
                <td>{{ $data->project->description }}</td>
                <td>{{ $data->location->latitude }}</td>
                <td>{{ $data->location->longitude }}</td>
                <td>{{ $data->date->start_project }}</td>
                <td>{{ $data->date->end_project }}</td>
                <td>{{ $data->budget->total_budget }}</td>
                <td>{{ $data->project->project_status }}</td>
