...
@section('content')
  ...
  <div class="content">
    <div class="card">
      ...
      <div class="card-body">
        <a href="{{ route('budgets.create') }}" type="button" class="btn btn-primary mb-3">Tambah Data</a>
        <a href="{{ route('cetakBudget') }}" target="_blank" type="button" class="btn btn-success mb-3"><i
      ...
      <form action="{{ route('cetakMonthlyBudgets') }}" target="_blank" method="GET" class="form-inline mb-3">
        ...
        <table id="budgets" class="table table-bordered  table-striped mb-3">
          <thead>
            <tr align="center">
              <th>NO</th>
              <th>Project</th>
          ...
          <tbody>
            @foreach ($budgets as $data)
              <tr align="center">
                ...
                <td>
                  <a href="{{ route('budgets.show', $data->id) }}"> <button class="btn btn-light m-2"><i
                        class="bi bi-eye-fill"></i></button></a>
                  <a href="{{ route('budgets.edit', $data->id) }}"> <button class="btn btn-secondary m-2"><i
                        class="bi bi-pencil-square"></i></button></a>
                        <a href="{{ route('cetakBudgetById', $data->id) }}" target="_blank" type="button"
                          class="btn btn-success m-2"><i class="bi bi-printer"></i></a> 
                  <form action="{{ route('budgets.destroy', $data->id) }}" method="POST"
