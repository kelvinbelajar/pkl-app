...
@section('content')
  ...
  <div class="content">
    <div class="card">
      ...
      <div class="card-body">
        <a href="{{ route('sources.create') }}" type="button" class="btn btn-primary mb-3">Tambah Data</a>
        <a href="{{ route('cetakSource') }}" target="_blank" type="button" class="btn btn-success mb-3"><i
      ...
      <form action="{{ route('cetakMonthlySources') }}" target="_blank" method="GET" class="form-inline mb-3">
        ...
        <table id="sources" class="table table-bordered table-striped mb-3">
          <thead>
            <tr align="center">
              <th>NO</th>
              <th>Project</th>
              <th>Source Organization Name</th>
              <th>Source Organization Manager</th>
              <th>Source Organization Email</th>
              <th>Aksi</th>   
            </tr>
          </thead>
          <tbody>
            @foreach ($sources as $data)
              <tr align="center">
                ...
                <td>{{ $data->source_manager }}</td>
                <td>{{ $data->source_email }}</td>
                
                <td>
                  <a href="{{ route('sources.show', $data->id) }}"><button class="btn btn-light m-2"><i
                        class="bi bi-eye-fill"></i></button></a>
                  <a href="{{ route('sources.edit', $data->id) }}"><button class="btn btn-secondary m-2"><i
                        class="bi bi-pencil-square"></i></button></a>
                        <a href="{{ route('cetakSourceById', $data->id) }}" target="_blank" type="button"
